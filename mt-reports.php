			...
			$table_top    = "<table class='widefat'><caption>" . sprintf( __( 'Tickets Purchased for &ldquo;%s&rdquo;', 'my-tickets' ), $title ) . "</caption>
						<thead>
							<tr>
								...
								<th scope='col'>" . __( 'Used', 'my-tickets' ) . "</th>
							</tr>
						</thead>
						<tbody>";
...
function mt_get_tickets( $event_id ) {
	...
	foreach ( $query as $ticket_id ) {
		...
		$first_name                  = get_post_meta( $purchase_id, '_first_name', true );
		$last_name                   = get_post_meta( $purchase_id, '_last_name', true );
		$used                        = ( in_array( $ticket_id, $used_tickets ) ) ? 'Used' : '';
		if ( !$first_name || !$last_name ) {
			$name = explode( ' ', $purchaser );
			$first_name = $name[0];
			$last_name = end( $name );
		...
		$alternate                   = ( $alternate == 'alternate' ) ? 'even' : 'alternate';
		$row                         = "<tr class='$alternate'><th scope='row'>$ticket_id</th><td>$type</td><td>$purchaser</td><td><a href='" . get_edit_post_link( $purchase_id ) . "'>$purchase_id</a></td><td>" . apply_filters( 'mt_money_format', $price ) . "</td><td>$used</td></tr>";
		// add split field to csv headers
	...
	if ( $event_id ) {
		...
		foreach ( $purchasers as $purchaser ) {
			if ( $purchaser['opt_out'] != 'true' ) {
				...
				$opt_out     = apply_filters( 'mt_opt_out_text', $opt_out, $opt_out_url, $post_id, 'bulk' );
				$to          = $purchaser['email'];
				$name        = $purchaser['name'];
