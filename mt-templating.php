...
function mt_ticket_purchase_id() {
	echo mt_get_ticket_purchase_id();
}
 ...
 *
 */
function mt_get_ticket_custom_field( $field = false, $callback = false ) {
	if ( $field ) {
		$purchase    = get_post_meta( mt_get_ticket()->ID, '_'.mt_get_ticket_id(), true );
		$purchase_id = $purchase['purchase_id'];
...
function mt_ticket_custom_field( $field = false ) {
	echo mt_get_ticket_custom_field( $field );
}