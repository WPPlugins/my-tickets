...
function mt_handle_cart() {
	...
	if ( ! isset( $_POST['mt_submit'] ) ) {
		...
		if ( !isset( $_POST['mt_fname'] )
            || $_POST['mt_fname'] == ''
            || !isset( $_POST['mt_lname'] )
            || $_POST['mt_lname'] == ''
            || !isset( $_POST['mt_email'] )
            || $_POST['mt_email'] == ''
            || !isset( $_POST['mt_email2'] )
            || $_POST['mt_email'] != $_POST['mt_email2'] ) {
		...
		if ( $payment ) {
			...
			if (
				isset( $_POST['mt_gateway'] ) && $_POST['mt_gateway'] == 'offline' &&
				( !isset( $_POST['ticketing_method'] ) || $_POST['ticketing_method'] != 'postal' )
				&& !mt_always_collect_shipping() ) {
